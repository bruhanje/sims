...
async def get_course_list(request):
    with db_block() as db:
        db.execute("""
        SELECT sn AS cou_sn, no AS cou_no, name AS cou_name, place, time FROM course
        """)
...
async def new_course(request):
    ...
    if not course.get('place'):
        course['place'] = '一公教'

    with db_block() as db:
        db.execute("""
        INSERT INTO course (no, name, place, time)
        VALUES(%(cou_no)s, %(cou_name)s, %(place)s, %(time)s) RETURNING sn;
        """, course)
        record = db.fetch_first()
...
async def update_course(request):
    ...
    with db_block() as db:
        db.execute("""
        UPDATE course SET
            no=%(cou_no)s, name=%(cou_name)s, place=%(place)s, time=%(time)s
        WHERE sn=%(cou_sn)s;
        """, course)
